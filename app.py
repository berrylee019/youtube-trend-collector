...
# Create directory structure for Streamlit app repo
os.makedirs("streamlit_youtube_collector", exist_ok=True)
...
with st.sidebar:
    ...
    default_api_key = st.secrets.get("YOUTUBE_API_KEY", "") if "YOUTUBE_API_KEY" in st.secrets else ""
    
    api_key = st.text_input("YouTube API Key", value=default_api_key, type="password")
    search_keyword = st.text_input("검색 키워드", value="Korea vlog")
    max_results = st.slider("수집할 영상 수", min_value=10, max_value=50, value=20)
    min_ratio = st.number_input("최소 성과 비율 (조회수/구독자수 %)", value=200, step=50)
    st.markdown("---")
...
if start_button:
    if not api_key:
        st.error("YouTube API Key를 입력해주세요! (Streamlit Secrets 설정 또는 사이드바 입력)")
    else:
            ...
            try:
                ...
                if not video_ids:
                    ...
                    for item in videos_response.get('items', []):
                        ...
                        ratio = round((view_count / sub_count) * 100, 1) if sub_count > 0 else 0
                    ...
                    if not data:
                        ...
                        st.success(f"총 {len(df)}개의 떡상 영상을 성공적으로 추출했습니다!")
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "구독자수": st.column_config.NumberColumn("구독자수", format="%d 명"),
                                "조회수": st.column_config.NumberColumn("조회수", format="%d 회"),
                                "성과비율(%)": st.column_config.NumberColumn("성과비율", format="%.1f%%"),
                            },
...
with open("streamlit_youtube_collector/app.py", "w", encoding="utf-8") as f:
...
with open("streamlit_youtube_collector/requirements.txt", "w", encoding="utf-8") as f:
    f.write(req_content)
...
with open("streamlit_youtube_collector/.gitignore", "w", encoding="utf-8") as f:
    f.write(gitignore_content)

# 4. README.md
readme_content = '''# 🎬 유튜브 떡상 영상 수집기 (Streamlit)

구독자 수 대비 높은 조회수를 기록한 유튜브 영상(외국인 반응, 브이로그 등)을 자동 추출해주는 Streamlit 애플리케이션입니다.

## 🚀 기능
- 키워드 기반 유튜브 영상 탐색 (YouTube Data API v3)
- 구독자 수 대비 조회수 비율(%) 자동 계산 및 등급 부여 (S/A/B)
- 실시간 썸네일 및 영상 정보 필터링
- CSV 데이터 다운로드 지원
